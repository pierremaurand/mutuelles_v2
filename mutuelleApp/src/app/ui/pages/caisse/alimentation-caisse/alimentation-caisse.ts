...
@Component({
  ...
  templateUrl: './alimentation-caisse.html',
  styleUrl: './alimentation-caisse.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export default class AlimentationCaisse {
  request!: FormGroup;
  caisseId!: number;
  ...
  onBanqueChange($event: any): void {
    ...
    console.log(banque);
    if (banque) {
      this.libelle = 'Retrait à la banque ' + banque.nom;
      this.request
        .get('montantCredit')
        ?.setValidators([
          Validators.required,
          Validators.min(0),
          Validators.max(banque.solde ?? 0),
        ]);
    }
